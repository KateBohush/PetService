  ...
  class Builder
    class << self
      ...
      def stop_test(suite, test, result = {})
        suites[suite][:tests][test][:steps].each do |step_title, step|
        ...
        MUTEX.synchronize do
          ...
          suites[suite][:tests][test][:stop] = timestamp(result[:finished_at])
          suites[suite][:tests][test][:start] = timestamp(result[:started_at]) if result[:started_at]
          suites[suite][:tests][test][:status] = result[:status]
          if result[:status].to_sym != :passed
            suites[suite][:tests][test][:failure] = {
              stacktrace: ((result[:exception]&.backtrace) || []).map(&:to_s).join("\r\n"),
      ...
      def start_step(suite, test, step)
        MUTEX.synchronize do
          ...
          suites[suite][:tests][test][:steps][step] = {
            title: step,
            start: timestamp,
            attachments: []
          }
      ...
      def add_attachment(suite, test, opts = {step: nil, file: nil, mime_type: nil})
        ...
        if step.nil?
          suites[suite][:tests][test][:attachments] << attach
        else
          suites[suite][:tests][test][:steps][step][:attachments] << attach
        end
      end
      ...
      def stop_step(suite, test, step, status = :passed)
        MUTEX.synchronize do
          ...
          suites[suite][:tests][test][:steps][step][:stop] = timestamp
          suites[suite][:tests][test][:steps][step][:status] = status
        end
      end
      ...
      def stop_suite(title)
        ...
        MUTEX.synchronize do
          ...
          suites[title][:stop] = timestamp
        end
      end

      def build!(_opts = {})
        suites_xml = []
        (suites || []).each do |suite_title, suite|
            ...
            xml.send 'ns2:test-suite', :start => suite[:start] || 0, :stop => suite[:stop] || 0, 'xmlns' => '', 'xmlns:ns2' => 'urn:model.allure.qatools.yandex.ru' do
              ...
              xml.send 'test-cases' do
                suite[:tests].each do |test_title, test|
                  t_set = nil
                  RSpec.configuration.tests.each do |t|
                    if test_title.include?(t.description)
                      t_set = t
                      test[:labels][:issue] = t.metadata[:issue] if t.metadata[:issue]
                    end
                  end
                  test_title = "#{test_title} #{t_set.metadata[:message]}" if t_set && t_set.metadata[:message]
      ...
      def validate_xml(xml)
        xsd = Nokogiri::XML::Schema(File.read(Pathname.new(File.dirname(__FILE__)).join("../../config/allure-model-#{AllureRubyAdaptorApi::Version::ALLURE}.xsd")))
